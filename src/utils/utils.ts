...
const integerErrorMsg =
  'One or more search criteria have an integer field but the value is a string. Adjust the value to be a valid integer.';
export const partialQueryErrorMsg =
  'One or more values are missing from the search criteria. Make sure all fields have values.';

const hasNoSearchParameters = (criteria: Array<CriteriaType>) => {
  const firstCriteria = criteria[0];
  if (
    criteria.length === 1 &&
    !firstCriteria.fieldId &&
    !firstCriteria.operatorId &&
  ...
  const missingCriteria = !!criteria.find((item) => {
    if (item.operatorId) {
      const operator = operators.find(
        (operator) => operator.id === item.operatorId,
      );
      if (operator && operator.operatorValue === BETWEEN) {
        return (
          !item.fieldId ||
          !item.operatorId ||
          !item.values[0] ||
